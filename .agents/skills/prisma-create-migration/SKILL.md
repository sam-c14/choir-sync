...
## When to use this
Whenever `choir-api/src/prisma/schema.prisma` needs a new model, field, or index.
...
2. Create the migration with a descriptive name: `npx prisma migrate dev --name <descriptive_name> --schema=choir-api/src/prisma/schema.prisma`.
...
4. Regenerate the client: `npx prisma generate --schema=choir-api/src/prisma/schema.prisma`.

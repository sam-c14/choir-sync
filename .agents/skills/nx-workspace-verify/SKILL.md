...
3. **Schema drift check** (this is the one that matters most given the "always use shared Zod schemas" rule): grep for any locally-redefined validation schema that duplicates something in `libs/shared/validation`: `grep -rn "z.object(" apps/choir-api/src apps/choir-client/app --include=".ts" --include=".tsx"`. Every hit should be inside `libs/shared/validation`, or a clear justification is required — a `z.object` in a controller or a form component is a sign the shared schema was bypassed.

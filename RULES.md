...
- Workspace: Nx Monorepo (apps/choir-client, apps/choir-api, libs/shared).
...
- Never modify `apps/choir-api/src/prisma/schema.prisma` without first flagging the change and getting explicit approval — schema changes ripple into the shared Zod schemas, the API, and the frontend.
